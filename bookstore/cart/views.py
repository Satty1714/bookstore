...
#前端发过来的数据：商品id 商品数目 book_id books_count
#涉及到数据的修改，使用post方式
def cart_add(request):
    ...
    coon = get_redis_connection('default')#default为settings中设置的
    cart_key = 'cart_%d' % request.session.get('passport_id')

    res = coon.hget(cart_key,book_id)
    if res is None:
    ...
    if res > books.stock:
        ...
        coon.hset(cart_key,book_id,res)
    #返回结果
...
def cart_show(request):
    ...
    for id,count in res_dict.items():
        #根据id获取商品的信息
        books = Books.objects.get_books_by_id(book_id=id)
        #保存商品的数目
        books.count = count
        #保存商品的小计
...
def cart_del(request):
    ...
    conn = get_redis_connection('default')
    cart_key = 'cart_%d' % request.session.get('passport_id')
    conn.hdel(cart_key, book_id)

    # 返回信息
    return JsonResponse({'res': 3})
# 前端传过来的参数:商品id book_id 更新数目 books_count
# post
# /cart/update/
def cart_update(request):
    ...
    conn.hset(cart_key, book_id, books_count)
